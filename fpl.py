...
def extract_teams_data(overview_data_json):
    ...
    teams_df = teams_df[['code', 'name', 'short_name', 'position', 'played', 'win', 'draw', \
                         'loss', 'points']]
    teams_df['past_position'] = pd.Series([2, 7, 15, 9, 6, 99, 12, 11, 17, 10, 5, 99, 1, 3, \
                                           4, 16, 99, 8, 14, 13])
    return teams_df
...
def add_players_using_lp(metric, costs, player_type, team, budget, team_counts,\
    ...
    try:
    	...
    	st.info('Data for the past gameweek has not been updated. Please try other inputs or come back later')
